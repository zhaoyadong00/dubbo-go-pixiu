...
func (z *ZooKeeperClient) Exists(path string) (bool, *zk.Stat, error) {
	conn := z.getConn()
	if conn == nil {
		return false, nil, errors.New("ZooKeeper client has no connection")
	}
...
type WatchManager struct {
	...
	wlock    sync.Mutex
	watchMap map[WatchPath]*PiWatchEventHandler
}
...
func (wm *WatchManager) key(path string) WatchPath {
	return WatchPath{path}
}

func (wm *WatchManager) getWatch(path string) *PiWatchEventHandler {
